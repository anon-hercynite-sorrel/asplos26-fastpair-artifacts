...
pub(crate) const FSST12_SAMPLETARGET: usize = 1 << 20;
...
pub(crate) fn make_sample<'a>(str_in: &[&'a [u8]], tot_size: usize) -> Vec<&'a [u8]> {
    make_sample_with_limit(str_in, tot_size, FSST_SAMPLETARGET)
}

/// As [`make_sample`], but with an explicit sample-size budget. FSST12 needs a larger
/// sample than FSST8: it has 16x the code space to fill, and a 16 KB sample cannot
/// support learning thousands of distinct symbols.
pub(crate) fn make_sample_with_limit<'a>(
    str_in: &[&'a [u8]],
    tot_size: usize,
    sample_lim: usize,
) -> Vec<&'a [u8]> {
