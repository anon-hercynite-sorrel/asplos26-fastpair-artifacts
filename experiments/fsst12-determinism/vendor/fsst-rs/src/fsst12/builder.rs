...
use crate::{
    Symbol,
    builder::{FSST12_SAMPLETARGET, fsst_hash, make_sample_with_limit},
    compare_masked,
};
...
impl Compressor12 {
    ...
    pub fn train(values: &[&[u8]]) -> Self {
        ...
        // Diagnostic override so the training-budget sweep can be run without a rebuild.
        let sample_target: usize = std::env::var("FSST12_SAMPLE_TARGET")
            .ok()
            .and_then(|v| v.parse().ok())
            .unwrap_or(FSST12_SAMPLETARGET);
        let sampled = tot_size >= sample_target;
        let sample = make_sample_with_limit(values, tot_size, sample_target);
