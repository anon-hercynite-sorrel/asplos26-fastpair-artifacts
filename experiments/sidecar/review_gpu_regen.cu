...
//
// review_gpu_regen.cu -- reviewer adaptation of public op_gpu_regen.cu.
// Original: anonymous harness f6d81878715bf322a0917ada32bcf40adffbd3ef,
// benchmarks/onpair-bench/op_gpu_regen.cu.
//
// A complete offset-regeneration versus preloaded-offset decode comparison for
// the PAPER recommended packed decoder: K configurable (4 or6), T256/B4/H1/S16,
// W configurable (8 or16), and offset batches exactly32*K codes.
// The public original reduction (512-thread blocks) + CUB exclusive scan is
// retained; decode uses the unchanged public onpair_decompress_tpt.cuh header.
// This is an explicit adaptation because the historical op_gpu_regen_grid.cu
// producer is absent from the published harness snapshot.
//
// Input E2E1: magic[4], token_count:u64, dict_size:u32, max_token:u32,
// codes:u16[token_count], lengths:u8[dict_size], padded_dict:u8[16*dict_size].
// Build: nvcc -O3 -arch=sm_103 -std=c++17 -I<public-harness>/vortex-cuda/kernels/src
//        -DREVIEW_K=6 -DREVIEW_W=16 review_gpu_regen.cu -o regen_k6_w16
// Run:   ./regen_k6_w16 <one-preset.e2ebin> 200
// Records raw CUDA-event arrays for regen, decode, and back-to-back regen+decode.
// Exit0 requires exact offsets, both decoded outputs, and untouched tail guards.
...
// The public parameterized packed decode kernel, unchanged.
// Reviewer adaptation of the public OP2 driver. Original reduction/CUB path retained.
// Compile -I<public-harness>/vortex-cuda/kernels/src -DREVIEW_K=6 -DREVIEW_W=16.
#ifndef REVIEW_K
#define REVIEW_K 6
#endif
#ifndef REVIEW_W
#define REVIEW_W 16
#endif
#define TOKENS_PER_THREAD REVIEW_K
#define ONPAIR_BLOCK_THREADS 256u
#define ONPAIR_MIN_BLOCKS 4u
#define ONPAIR_LOW_PLANE_BYTES REVIEW_W
#define ONPAIR_HELD_HIGH 1u
#define ONPAIR_STAGE_BYTES 16u
#define ONPAIR_KERNEL_NAME review_packed_decode
...
// One offset per32*K codes. The compile-time assertion binds the scan and decode
// granularity; unlike the public original K4-only driver, decode remains enabled
// at K6/192. Regeneration retains its independent512-thread launch geometry.
#ifndef TOK_PER_BATCH_OVERRIDE
#define TOK_PER_BATCH_OVERRIDE (32 * REVIEW_K)
#endif
static constexpr uint32_t TOK_PER_BATCH = TOK_PER_BATCH_OVERRIDE;
static_assert(TOK_PER_BATCH_OVERRIDE == 32 * REVIEW_K, "offset granularity must match decode K");
static constexpr bool DECODE_VALID = true;
static constexpr int DECODE_BLOCK_THREADS = 256;
static constexpr int BLOCK_THREADS = 512;               // 16 warps/block
static constexpr uint32_t WARPS_PER_BLOCK = BLOCK_THREADS / 32;

// Per-batch decoded-size reduction: one warp per TOK_PER_BATCH-token batch sums lens[codes[t]] over its
// (up to) TOK_PER_BATCH tokens and writes the total to batchsize[b]. Coalesced code reads (lane-consecutive);
...
int main(int argc, char **argv) {
  ...
  // Bound the ceiling division and CUB int item count: token_count<=INT_MAX*TOK_PER_BATCH.
  ...
  // Match the public packed kernel ABI; the regeneration LUT remains byte lengths.
  std::vector<uint8_t> packed_lens((dict_size + 1u) / 2u, 0);
  std::vector<uint8_t> dict_hi((size_t)dict_size * 8u, 0);
  for (uint16_t c : codes) if (lens[c] == 0) {
    fprintf(stderr, "referenced zero-length token violates packed decoder ABI\n"); return 2;
  }
  for (uint32_t c = 0; c < dict_size; ++c) {
    uint32_t n = lens[c] ? lens[c] : 1u;
    packed_lens[c / 2u] |= (uint8_t)((n - 1u) << (4u * (c & 1u)));
    memcpy(dict_hi.data() + (size_t)c * 8u, dict_padded.data() + (size_t)c * 16u + 8u, 8u);
  }

  // ── upload ──────────────────────────────────────────────────────────────────
  uint16_t *d_codes;
  uint8_t *d_lens, *d_s8, *d_padded, *d_out, *d_hi, *d_packed;
  ...
  CK(cudaMalloc(&d_packed, packed_lens.size()));
  CK(cudaMemcpy(d_hi, dict_hi.data(), dict_hi.size(), cudaMemcpyHostToDevice));
  CK(cudaMemcpy(d_packed, packed_lens.data(), packed_lens.size(), cudaMemcpyHostToDevice));
  ...
  const uint32_t dec_grid = (uint32_t)((n_chunks + DECODE_BLOCK_THREADS / 32 - 1) / (DECODE_BLOCK_THREADS / 32));
  ...
  auto decode = [&](uint64_t *choff) {
    review_packed_decode<<<dec_grid, DECODE_BLOCK_THREADS>>>(
        d_codes, choff, REVIEW_W == 16 ? d_padded : d_s8, d_hi, d_packed, d_out, total_tokens);
  };
  ...
  // Independently validate preloaded-offset baseline and protect the output tail.
  bool baseline_ok = false, guard_ok = true;
  unsigned char guard[64];
  CK(cudaMemcpy(guard, d_out + decoded_bytes, 64, cudaMemcpyDeviceToHost));
  for (auto v : guard) guard_ok = guard_ok && v == 0xA5;
  CK(cudaMemset(d_out, 0xA5, decoded_bytes + 64));
  decode(d_choff_ref);
  CK(cudaDeviceSynchronize()); CK(cudaGetLastError());
  std::vector<uint8_t> baseline_out(decoded_bytes);
  CK(cudaMemcpy(baseline_out.data(), d_out, decoded_bytes, cudaMemcpyDeviceToHost));
  baseline_ok = memcmp(baseline_out.data(), cpu_out.data(), decoded_bytes) == 0;
  CK(cudaMemcpy(guard, d_out + decoded_bytes, 64, cudaMemcpyDeviceToHost));
  for (auto v : guard) guard_ok = guard_ok && v == 0xA5;
  decode_ok = decode_ok && baseline_ok && guard_ok;
  ...
  printf("  \"gpu\": \"%s\",\n", prop.name);
  printf("  \"kernel\": \"review_packed_decode + public gpu-regen-offsets\",\n");
  printf("  \"k\": %u, \"block_threads\": 256, \"min_blocks\": 4, \"held_high\": 1, \"stage_bytes\": 16,\n", REVIEW_K);
  printf("  \"low_plane_bytes\": %u, \"regen_block_threads\": %u, \"dict_size\": %u,\n", REVIEW_W, BLOCK_THREADS, dict_size);
  printf("  \"baseline_ok\": %s, \"guard_ok\": %s,\n", baseline_ok ? "true" : "false", guard_ok ? "true" : "false");
  ...
  // Decode and regeneration have matching batches; require every validation.
